defmodule ZebraPuzzle do
  ...
  defp puzzle() do
    ...
    domains |> propagate(@objects, constraints) |> guess(@objects, constraints)
  end
  ...
  defp assert(domains, objects, [{x, x_val}, {y, y_val}]) do
  ...
  defp refute(domains, [{x, x_val}, {y, y_val}]) do
    domains
    |> update_in([x, x_val, y], &List.delete(&1, y_val))
    |> update_in([y, y_val, x], &List.delete(&1, x_val))
  end

  defp propagate(domains, objects, constraints) do
  ...
  defp guess(domains, objects, constraints) do
