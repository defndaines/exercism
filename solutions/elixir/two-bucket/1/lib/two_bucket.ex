defmodule TwoBucket do
  ...
  # TODO: 19 is based solely on tests, but number should be derivable.
  defp do_measure(%__MODULE__{moves: 19}, _, _, _, _), do: {:error, :impossible}

  defp do_measure(%{bucket_one: 0, bucket_two: max_two} = bs, :one, max_one, max_two, goal) do
    do_measure(fill(bs, :one, max_one), :one, max_one, max_two, goal)
  end

  defp do_measure(%{bucket_one: max_one, bucket_two: 0} = bs, :two, max_one, max_two, goal) do
    do_measure(fill(bs, :two, max_two), :two, max_one, max_two, goal)
  end

  defp do_measure(%__MODULE__{bucket_one: goal} = bs, _, _, _, goal), do: {:ok, bs}
  defp do_measure(%__MODULE__{bucket_two: goal} = bs, _, _, _, goal), do: {:ok, bs}
  ...
  defp fill(bs, :one, max) do
    %__MODULE__{bs | bucket_one: max, moves: bs.moves + 1}
  end

  defp fill(bs, :two, max) do
    %__MODULE__{bs | bucket_two: max, moves: bs.moves + 1}
  end
end
