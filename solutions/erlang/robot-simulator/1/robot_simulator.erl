...
-record(state, { direction = undefined
...
handle_call({place, Direction, Position}, _From, _State) ->
  {reply, ok, #state{ direction = Direction, position = Position}};
...
place(robot, Dir, Position) ->
  gen_server:call(robot, {place, Dir, Position}).
